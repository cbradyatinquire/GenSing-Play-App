...
public class Application extends Controller {
    ...
    public static void CommentsForWaveState( Long id, String comments )
    {
    	WaveSaveState thestate = WaveSaveState.findWaveSaveState(id);
  ...
  //test methods...
    ...
    public static void assessContributions( String cids, String eq, String hits ) {
        String reply = "Setting target equation and isHits - ";
        String[] assessedCids = cids.split( "," );
        String assessedEq = eq;
        String[] assessedHits = hits.split( "," );

        if( assessedCids.length != assessedHits.length ) {
            renderJSON( reply + "FAIL. Number of contributions and number of hits do not tally" );
        } else {
            for( int i = 0; i < assessedCids.length; i++ ) {
                Long cid = Long.decode( assessedCids[ i ] );
                String isHit = assessedHits[ i ];
                Contribution c = Contribution.getContribution( cid );
                c.target = assessedEq;
                c.isHit = isHit;
                c.save();
            }
